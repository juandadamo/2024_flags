...
nymin , nymax, nxmin, nxmax = [323,753,343,600]
nyorigin = 559
nyorigin = 194
# nsnapshots = 100
...
def sine_func(x, amplitude, frequency, phase, offset):
    """Sine function: y = A * sin(2πf*x + φ) + C"""
    return amplitude * np.sin(2 * np.pi * frequency * x + phase) + offset
x = np.arange(len(U[:,0]))
y_noisy = U[:,0]
initial_guess = [U[:,0].max(),1/360,0,0]
params, covariance = curve_fit(sine_func, x, y_noisy, p0=initial_guess)
