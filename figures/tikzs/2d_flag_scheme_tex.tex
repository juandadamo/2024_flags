...
\begin{document}
	\begin{tikzpicture}
		\node(fig_2dd) at(0,0) {\includegraphics[width=10cm,trim=0 2cm 0cm 1.7cm,clip]{2D_flag_snapshots}};
		\node(fig_2d) at(-6,1.1){};
...
%		\fill[white](fig_2d.west)++(0,0)rectangle++(1,-1);
		...
		\foreach \y in {0.3,0.6,...,2}{
			\draw[->,thick](fig_2d.west)++(1cm,-.75+ \y)--++(1.4,0);  }
		\draw[->,thick](fig_2d.west)++(1,-0.8)--++(0.98,0)node(cp1){};
