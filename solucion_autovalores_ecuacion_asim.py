...
def u_profile(s, u0, shear_type='tanh', R=1, theta=1):
    """
    Perfiles de velocidad con shear tipo tanh.

    s : posición adimensional [0,1]
    u0 : velocidad en s=0 (base)
    shear_type : 'uniform' o 'tanh'
    delta : ancho de transición (0.1 = abrupto, 0.5 = suave)
    u_ratio : u(1)/u(0) (ej: 0.5 = extremo libre a mitad de velocidad)
    """
    if shear_type == 'uniform':
        return u0 * np.ones_like(s)

    elif shear_type == 'tanh':
        # Transición suave entre u0 (base) y u0*u_ratio (extremo libre)
        u_max = u0                # Velocidad en base (s=0)
        u_min = u0 * u_ratio      # Velocidad en extremo libre (s=1)
        s0 = 0.5                  # Centro de la transición

        # Perfil tanh: u_min + (u_max - u_min)*0.5*(1 - tanh((s - s0)/delta))
        return u_min + (u_max - u_min) * 0.5 * (1 - np.tanh((s - s0)/delta))
